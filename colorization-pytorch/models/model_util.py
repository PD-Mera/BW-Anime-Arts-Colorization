...
#     nax = torch_setdiff1d(torch.arange(0, NDIM), torch.tensor([axis]))
#     # nax = torch.setdiff(torch.arange(0, NDIM), torch.tensor((axis)))  # non axis indices
#     NPTS = torch.prod(SHP[nax])
#     axorder = torch.cat((nax, torch.tensor([axis]).flatten()), dim=0)
#     pts_flt = pts_nd.permute([int(i) for i in axorder])
#     pts_flt = pts_flt.reshape(NPTS, SHP[axis])
#     print(pts_flt.size())
#     return pts_flt
