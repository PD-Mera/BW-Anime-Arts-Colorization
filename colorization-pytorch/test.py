...
if __name__ == '__main__':
    ...
    dataset = torchvision.datasets.ImageFolder(opt.dataroot,
                                               transform=transforms.Compose([
                                                   transforms.Resize((opt.loadSize, opt.loadSize)), # opt.loadSize = 256
                                                   transforms.ToTensor()]))
    ...
    # statistics
    psnrs = np.zeros((opt.how_many, S)) # opt.how_many = 200
    entrs = np.zeros((opt.how_many, S))

    for i, data_raw in enumerate(dataset_loader):
        # data_raw[0] is image, data_raw[1] is label but didn't use in this project
        data_raw[0] = data_raw[0].cuda()
        data_raw[0] = util.crop_mult(data_raw[0], mult=8)

        # with no points
        for (pp, sample_p) in enumerate(sample_ps):
            img_path = ['{:08d}_{:0.3f}'.format(i, sample_p).replace('.', 'p')]
